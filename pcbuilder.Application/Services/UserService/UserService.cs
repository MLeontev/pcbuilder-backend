...
public class UserService : IUserService
{
    ...
    public async Task<Result<AuthResult>> RegisterUser(string username, string password)
    {
        ...
        if (existingUser != null) return Result.Failure<AuthResult>(UserErrors.UsernameTaken);
    ...
    public async Task<Result<AuthResult>> Login(string username, string password)
    {
        ...
        if (user == null) return Result.Failure<AuthResult>(UserErrors.InvalidCredentials);

        var passwordValid = await _userManager.CheckPasswordAsync(user, password);

        if (!passwordValid) return Result.Failure<AuthResult>(UserErrors.InvalidCredentials);
    ...
    public async Task<Result> Logout(string refreshToken)
    {
        ...
        if (user == null) return Result.Failure(UserErrors.InvalidToken);
